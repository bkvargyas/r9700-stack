"""A-tiled prefill path (fragment-tiled activations, r9k_quant_rows_fp8_tiled + r9k_moe_4bit_prefill_at):
  ...
  (b) every A-tiled cfg on a mid shape (ragged M and N tails) against the dequantized reference AND bit-identical to
      the folded LDS-A prefill kernel (same WMMA accumulation order, so the outputs must match exactly);
  (c) the served 27B shapes on the cfg pick_cfg selects, plus every tuned.json "mxfp4_at" row;
  (d) the public wrappers the model calls: ops.mxfp4_linear (torch.ops.r9700.mxfp4_linear) at decode and prefill M with
      fold on/off, and R9700Mxfp4LinearKernel.apply_weights on a fake layer (the served path, needs vLLM).
...
def make_w(N, Kd):
    ...
    W = dataclasses.replace(K.prepare_mxfp4_weights(p[None], e[None]), fold=True)
    return W, mx_deq(p, e)
...
# ---- (b) every A-tiled cfg: vs reference and bit-identical to the folded LDS-A kernel (cfg 12 / 16 = same tiles)
...
for M in (256, 300, 1024):
    ...
    base = run_fold(W, q, s, M, 12)
    for cfg in sorted(K.ATILED_TILES):
        out = run_at(W, qt, s, M, cfg)
        check(f"atiled 1040x1024 M={M} cfg={cfg} {K.ATILED_TILES[cfg]}", out, ref)
        check_eq(f"atiled 1040x1024 M={M} cfg={cfg} == folded LDS-A cfg 12", out, base)
del W, wd

# ---- (c) served shapes on the picked cfg, and every tuned "mxfp4_at" row
...
for (N, Kd) in ((17408, 5120), (5120, 8704), (8240, 5120), (7168, 5120), (5120, 3072)):
    W, wd = make_w(N, Kd)
    cases = {M: K.pick_cfg(N, Kd, 32, M=M, kind="mxfp4", fold=True) for M in (512, 2048)}
    for Ms, cfg in T.get("mxfp4_at", {}).get(f"{N},{Kd}", {}).items():
        cases[int(Ms)] = tuple(cfg)
    for M, cfg in sorted(cases.items()):
        good = K.is_atiled_cfg(cfg)
        ok &= good
        print(f"  {'pick_cfg(%d,%d,M=%d,fold) -> %s' % (N, Kd, M, cfg):80s} {'ok' if good else '<-- FAIL (not A-tiled)'}")
        if not good:
            continue
        x = torch.randn(M, Kd, device="cuda", generator=g).to(torch.bfloat16)
        q, s = K.quant_rows_fp8(x)
        ...
        check(f"atiled dense {N}x{Kd} M={M} cfg={cfg}", out, (q.float() * s[:, None]) @ wd.T)
        check_eq(f"atiled dense {N}x{Kd} M={M} cfg={cfg} == folded LDS-A cfg 12", out, run_fold(W, q, s, M, 12))
    del W, wd
...
try:
    ...
    for M in (8, 64, 256, 1000, 2048):
        ...
        good = K.is_atiled_cfg(K.pick_cfg(N, Kd, M=M, kind="mxfp4", fold=True)) == (M >= K.PREFILL_MIN_M)
        ok &= good
        print(f"  {'dispatch M=%d: A-tiled iff M >= PREFILL_MIN_M' % M:80s} {'ok' if good else '<-- FAIL'}")
    ...
    try:
        ...
        check("R9700Mxfp4LinearKernel.apply_weights M=1500 fold (served path)", out, (q.float() * s[:, None]) @ wd.T)

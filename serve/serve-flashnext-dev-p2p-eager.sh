...
sudo docker run -d --name vllmflashnext --restart no --privileged --ipc=host --network=host \
  ...
  -e VLLM_PLE_CPU_OFFLOAD=1 -e HIP_VISIBLE_DEVICES=0,1 -e NCCL_PROTO=Simple \
  -v $HOME/rccl-build/librccl-final.so:/opt/rocm/core-10.0/lib/librccl.so.1.0:ro \
  -v $HOME/rccl-build/libnccldump_stub.so:/opt/rocm/core-10.0/lib/libnccldump_stub.so:ro \
  -v $HOME/rccl-build/libnccl_devapi_stub.so:/opt/rocm/core-10.0/lib/libnccl_devapi_stub.so:ro \
  ...
  --kv-cache-dtype fp8 --gpu-memory-utilization 0.97 --enforce-eager \
  --enable-expert-offload --expert-offload-mem 60 \

...
class R9700Mxfp4Experts(mk.FusedMoEExpertsModular):
    ...
    def apply(self, output, hidden_states, w1, w2, topk_weights, topk_ids, activation, global_num_experts,
        ...
        sorted_ids, expert_ids, ntpp = moe_align_block_size(topk_ids, K.MOE_BLOCK, global_num_experts,
                                                            expert_map)
        W1 = K.Mxfp4Experts(w1, self.w1_scale, N1, K1)
        W2 = K.Mxfp4Experts(w2, self.w2_scale, N2, K2)

        xq, xs = K.quant_rows_fp8(hidden_states)
        gate_up = torch.empty((numel, N1), dtype=torch.bfloat16, device=dev)
        K.moe_gemm(xq, xs, W1, gate_up, sorted_ids, expert_ids, ntpp, numel, topk, None, *CFG_GATE_UP,
        ...
        K.moe_gemm(aq, as_, W2, down, sorted_ids, expert_ids, ntpp, numel, 1, tw, *CFG_DOWN,
                   num_experts=global_num_experts)
